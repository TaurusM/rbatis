...
use crate::util::{find_fn_body, find_return_type, get_fn_args};
...
pub(crate) fn impl_macro_py_sql(target_fn: &ItemFn, args: ParseArgs) -> TokenStream {
    ...
    // }
    //append all args
    let sql_args_gen = filter_args_context_id("unused_rbatis_name", &get_fn_args(target_fn));
    ...
    return quote! {
       pub fn #func_name_ident #generic(#func_args_stream) -> #return_ty {
         ...
         #include_data
         #gen_func
         let (sql,rb_args) = do_py_sql(&rbs::Value::Map(rb_arg_map), '?');
         Ok((sql,rb_args))
       }
    }
    .into();
